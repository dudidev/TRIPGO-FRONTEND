...
// ── Recomendaciones ───────────────────────────────────
 ...
 getMiPerfil(): Observable<any> {
  return this.http.get(`${this.baseUrl}/recomendaciones/mi-perfil`);
}

refrescarPerfil(): Observable<any> {
  return this.http.post(`${this.baseUrl}/recomendaciones/refrescar-perfil`, {});
}

 registrarVisualizacion(idEstablecimiento: number, tiempoSegundos = 0): void {
  // ✅ Ya no chequea token — authMiddleware del backend valida la cookie
  this.http.post(
